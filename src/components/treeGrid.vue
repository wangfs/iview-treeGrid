                    ...
                    <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
                        ...
                        <label @click="toggle(index,item)" v-if="!column.type">
                            <span v-if='snum==iconRow()'>
                              ...
                              <i v-if="item.options&&item.options.length>0" class="ivu-icon" :class="{'ivu-icon-chevron-down':!item.expanded,'ivu-icon-chevron-up':item.expanded }"></i>
    ...
    methods: {
      ...
      initData (items, level, parent) {
        ...
        items.forEach((item, index) => {
          ...
          if (item.options && item.expanded) {
            this.initData(item.options, level + 1, item)
          }
        })
      },
      //  隐藏显示
        ...
        if (item.options) {
          if (item.expanded) {
            ...
            if (item.load) {
              ...
              item.load = true
              item.options.forEach((child, childIndex) => {
      ...
      open (index, item) {
        if (item.options) {
          item.options.forEach((child, childIndex) => {
            child.isShow = true
            if (child.options && child.expanded) {
      ...
      All (data) {
        ...
        data.forEach((item) => {
          ...
          if (item.options && item.options.length > 0) {
            arr = arr.concat(this.All(item.options))
          }
        })
        return arr
      },
      ...
      Length (data) {
        ...
        data.forEach((child) => {
          if (child.options) {
            length += this.Length(child.options)
          }
        })
        return length
      },
      ...
      renderCheck (data) {
        ...
        data.forEach((item) => {
          ...
          if (item.options && item.options.length > 0) {
            arr = arr.concat(this.renderCheck(item.options))
          }
        })
        return arr
      },
